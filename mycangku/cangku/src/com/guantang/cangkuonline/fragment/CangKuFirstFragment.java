		...
		searchEdit.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				if(!TextUtils.isEmpty(s.toString())){
					Intent intent = new Intent(context,HpManagerListActivity.class);
					intent.putExtra("scanOrsearch", 1);//1代表搜索，2代表扫描。
					intent.putExtra("searchString", s.toString());
...
//						mSharedPreferences);
					...
					if(cklist.size()>0){
						...
						while(it.hasNext()){
							...
							if(TextUtils.isEmpty(id)){
								...
								dm_op.insert_into1(DataBaseHelper.TB_CK, str_ck, values,db);
	...
	private void jpushRegister(String dwname,String username){
		...
		AliasAndTagsSetting aliasAndTagsSetting = new AliasAndTagsSetting(loginTags.returnTags(),companyAlias.returnAlias(),context,context.getApplicationContext().getMainLooper());
	...
	public void switchFunctionItem(String name) {
		...
		if (name.equals("新增货品")) {
			...
			if (mSharedPreferences.getInt(ShareprefenceBean.IS_LOGIN, -1) == 1) {
				if (WebserviceImport.isOpenNetwork(context)) {
					if (RightsHelper.isHaveRight(RightsHelper.dj_db_add,
							mSharedPreferences) == true) {
						intent.setClass(context, AddDiaoboActivity.class);
						startActivity(intent);
					} else {
								Toast.makeText(context, "对不起，你没有的新增调拨的权限",
										Toast.LENGTH_SHORT).show();
							}
			...
			if (mSharedPreferences.getInt(ShareprefenceBean.IS_LOGIN, -1) == 1) {
				if (WebserviceImport.isOpenNetwork(context)) {
					...
					ShareprefenceBean.IS_LOGIN, -1) == -1) {
				Toast.makeText(context, "离线模式下不能查看历史据",
						Toast.LENGTH_SHORT).show();
			}

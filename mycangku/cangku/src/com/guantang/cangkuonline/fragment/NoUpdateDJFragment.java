...
//				context);
//		builder.setMessage("对不起，你没有"+str+"的添加权限。");
...
//					}
	...
	class allUpLoadAsyncTask extends AsyncTask<List<Map<String,Object>>, Integer, Void>{
		...
		protected void onPostExecute(Void result) {
			...
			if(progress>0){
				if(fail>0){
					builder.setMessage("上传成功"+success+"笔单据，上传失败"+fail+"笔单据。可能没有相关权限,失败的的单据请单独上传！");
				}else{

...
public class ModifyHPActivity extends BaseActivity implements OnClickListener{
	...
	private EditText tmEditText, bmEditText, nameEditText, ggEditText,
			...
			res5EditText, res6EditText, bzEditText, yxqEditText;
	private String[] str={DataBaseHelper.ID,DataBaseHelper.HPMC,DataBaseHelper.HPBM,DataBaseHelper.HPTM,DataBaseHelper.GGXH,
			...
			DataBaseHelper.LBS,DataBaseHelper.LBID,DataBaseHelper.LBIndex,DataBaseHelper.KCJE,DataBaseHelper.ImagePath,"yxq"};
	private String[] str1={"id","HPMC","HPBM","HPTBM","GGXH","CurrKc","JLDW","HPSX","HPXX","SCCS","BZ","RKCKJ","CKCKJ",
			"CKCKJ2","JLDW2","BigNum","res1","res2","res3","res4","res5","res6","LBS","LBID","LBIndex","kcje","ImageUrl","yxq"};
	...
	public void setText(List<Map<String, Object>> list){
		...
		Object yxqObject = map.get("yxq");
		yxqEditText.setText(yxqObject==null?"":yxqObject.toString());
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				case 1:
					...
					dm.del_hp(hpid);
					dm.Addhp1(Integer.parseInt(hpid),nameEditText.getText().toString().trim(),
							...
							res6EditText.getText().toString().trim(), "", "", 0, "","",yxqEditText.getText().toString().trim());
					finish();
					break;
				case -1:
					Toast.makeText(ModifyHPActivity.this, jsonObject.getString("Message"), Toast.LENGTH_SHORT).show();
	...
	public String createJson(){
		...
		map.put("yxq", yxqEditText.getText().toString().trim());
	...
	public void onClick(View v) {
		...
		case R.id.ok:
			...
			if(TextUtils.isEmpty(bmEditText.getText().toString())){
				Toast.makeText(this, "货品编码不能为空", Toast.LENGTH_SHORT).show();
				return;
			}

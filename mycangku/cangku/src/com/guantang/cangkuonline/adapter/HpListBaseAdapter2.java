...
public class HpListBaseAdapter2 extends MyJudgePictureAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.picImageView = (ImageView) convertView.findViewById(R.id.picImgView);
		...
		holder.itemname.setText(hpmcObject==null||hpmcObject.toString().equals("null")?"":hpmcObject.toString());

		Object hpbmObject = map.get(DataBaseHelper.HPBM);
		holder.itemcode.setText(hpbmObject==null||hpmcObject.toString().equals("null")?"":hpbmObject.toString());

		Object ggxhObject = map.get(DataBaseHelper.GGXH);
		holder.itemgg.setText(ggxhObject==null||ggxhObject.toString().equals("null")?"":ggxhObject.toString());
		
		
		if(op_type==1){
			holder.kucunnameTextView.setText("货品总库存：");
			Object currkcObject = map.get(DataBaseHelper.CurrKC);
			holder.itemnum.setText((currkcObject==null ||  currkcObject.toString().endsWith("null"))?"0":DecimalsHelper.Transfloat(Double.parseDouble(currkcObject.toString()),MyApplication.getInstance().getNumPoint()));
		}else if(op_type==0||op_type==2){
			holder.kucunnameTextView.setText("本仓库库存：");
			Object kcslObject = map.get(DataBaseHelper.KCSL);
			holder.itemnum.setText((kcslObject==null || kcslObject.toString().endsWith("null"))?"0":DecimalsHelper.Transfloat(Double.parseDouble(kcslObject.toString()),MyApplication.getInstance().getNumPoint()));
		}
		...
		if(IS_LOGIN==1 && showPic){
			...
			.crossFade().into(holder.picImageView);

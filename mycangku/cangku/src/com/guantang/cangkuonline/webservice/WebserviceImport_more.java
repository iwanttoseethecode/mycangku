...
public class WebserviceImport_more {
	...
	public static String checkUsername(SharedPreferences mSharedPreferences){
		PwdHelper pwdhelper = new PwdHelper();
		if(mSharedPreferences.getInt(ShareprefenceBean.TIYANZHANGHAO, 0)==1){//是否是体验账户
			return "admin";
		}else{
			return mSharedPreferences.getString(ShareprefenceBean.USERNAME, "");
		}
	}
	
	public static String checkPassWord(SharedPreferences mSharedPreferences){
		PwdHelper pwdhelper = new PwdHelper();
		if(mSharedPreferences.getInt(ShareprefenceBean.TIYANZHANGHAO, 0)==1){//是否是体验账户
			return pwdhelper.createMD5("admin" + "#cd@guantang").toUpperCase();
		}else{
			return pwdhelper.createMD5(mSharedPreferences.getString(ShareprefenceBean.PASSWORD, "") + "#cd@guantang").toUpperCase();
		}
	}
...
//	}
	 ...
	 * */
	public static String Login_Validate_2_0(String Json){
		...
		httpTranstation.debug = true;
		try {
			httpTranstation.call("http://guantang.net/"+ WebserviceHelper.Login_Validate_2_0, envelope);
			Object result = envelope.getResponse();
			if(result!=null&&!result.toString().equals("anyType{}")){
				return result.toString();
			}else{
	...
	public static String AndroidApkUpdate(int version){
		SoapObject soapObject = new SoapObject("http://tempuri.org/", WebserviceHelper.ApkUpdate);
		SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
		soapObject.addProperty("version",version);
		soapObject.addProperty("Apktype","CK001");
		envelope.bodyOut = soapObject;
		envelope.dotNet = true;
		envelope.encodingStyle = "UTF-8";
		HttpTransportSE httpTranstation = new HttpTransportSE("http://www3.gtcangku.com/CheckUpdate.asmx", 15000);
//		HttpTransportSE httpTranstation = new HttpTransportSE("http://192.168.0.9/GtStorageWebservice.asmx");
		httpTranstation.debug = true;
		try {
			httpTranstation.call("http://tempuri.org/"+ WebserviceHelper.ApkUpdate, envelope);
			Object result = envelope.getResponse();
			if(result!=null&&!result.toString().equals("anyType{}")){
				return result.toString();
			}else{
	...
	public static String AddRuKu_2_0(String jsonstring){
		SharedPreferences mSharedPreferences = MyApplication.getInstance().getSharedPreferences();
		...
		envelope.headerOut = header;
		try {
			httpTransportation.call("http://guantang.net/"+WebserviceHelper.AddRuKu_2_0, envelope);
			Object result = envelope.getResponse();
			if(result!=null && !result.toString().equals("anyType{}")){
				return result.toString();
			}else{
	...
	public static String AddChuKu_2_0(String jsonstring,boolean isck){
		SharedPreferences mSharedPreferences = MyApplication.getInstance().getSharedPreferences();
		...
		envelope.headerOut = header;
		try {
			httpTransportation.call("http://guantang.net/"+WebserviceHelper.AddChuKu_2_0, envelope);
			Object result = envelope.getResponse();
			if(result!=null && !result.toString().equals("anyType{}")){
				return result.toString();
			}else{
	...
	public static String GetHtDDInfo_1_0(String type, String orderid, String top, String dt1, String dt2, String wherestr, String direction){
		SharedPreferences mSharedPreferences = MyApplication.getInstance().getSharedPreferences();
		...
		envelope.headerOut = header;
		try {
			httpTransportation.call("http://guantang.net/"+WebserviceHelper.GetHtDDInfo_1_0, envelope);
			Object result = envelope.getResponse();
			if(result!=null && !result.toString().equals("anyType{}")){
				return result.toString();
			}else{
	...
	public static String DDAudit_1_0(String DDid, String dirc, String Whether, String Mes){
		SharedPreferences mSharedPreferences = MyApplication.getInstance().getSharedPreferences();
		SoapObject soapObject = new SoapObject("http://guantang.net/", WebserviceHelper.DDAudit_1_0);
		SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
		soapObject.addProperty("DDid",DDid);
		soapObject.addProperty("dirc",dirc);
		soapObject.addProperty("Whether",Whether);
		soapObject.addProperty("Mes",Mes);
		soapObject.addProperty("SerId", mSharedPreferences.getString(ShareprefenceBean.SERID, ""));
		soapObject.addProperty("LoginFlag",mSharedPreferences.getInt(ShareprefenceBean.LOGIN_FLAG, 1));
		envelope.bodyOut = soapObject;
		envelope.dotNet = true;
		...
		envelope.headerOut = header;
		try {
			httpTransportation.call("http://guantang.net/"+WebserviceHelper.DDAudit_1_0, envelope);
			Object result = envelope.getResponse();
			if(result!=null && !result.toString().equals("anyType{}")){
				return result.toString();
			}else{
...
//		mOkHttpClient.newBuilder().connectTimeout(30, TimeUnit.SECONDS).readTimeout(15, TimeUnit.SECONDS).writeTimeout(15, TimeUnit.SECONDS).build();
...
//				.add("Whether",Whether)
//				.add("Mes",Mes)
//				.add("SerId", mSharedPreferences.getString(ShareprefenceBean.SERID, ""))
//				.add("LoginFlag",String.valueOf(mSharedPreferences.getInt(ShareprefenceBean.LOGIN_FLAG, 1)))
//				.build();
//		String ss = checkUsername(mSharedPreferences);
//		String sss = checkPassWord(mSharedPreferences);
//		Request request = new Request.Builder()
//				.post(requestBody)
//				.url(mSharedPreferences.getString(ShareprefenceBean.NET_URL, "")+WebserviceHelper.GtStorageWebservice+"/"+WebserviceHelper.DDAudit_1_0)
//				.addHeader("UserName", checkUsername(mSharedPreferences))
//				.addHeader("password",checkPassWord(mSharedPreferences))
//				.build();
//		
...
//							responseString=parser.nextText();
//						}
//						break;
